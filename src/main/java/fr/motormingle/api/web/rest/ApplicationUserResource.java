...
import fr.motormingle.api.repository.ApplicationUserRepository;
...
/**
 * REST controller for managing {@link fr.motormingle.api.domain.ApplicationUser}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ApplicationUserResource {
    ...
    public ResponseEntity<ApplicationUser> partialUpdateApplicationUser(
        ...
        Optional<ApplicationUser> result = applicationUserRepository
            ...
            .map(existingApplicationUser -> {
                if (applicationUser.getTag() != null) {
                    existingApplicationUser.setTag(applicationUser.getTag());
                }
                ...
                if (applicationUser.getBirthDate() != null) {
                    existingApplicationUser.setBirthDate(applicationUser.getBirthDate());
                }
    ...
    public List<ApplicationUser> getAllApplicationUsers(
        ...
        if ("ownership-is-null".equals(filter)) {
            log.debug("REST request to get all ApplicationUsers where ownership is null");
            return StreamSupport
                .stream(applicationUserRepository.findAll().spliterator(), false)
                .filter(applicationUser -> applicationUser.getOwnership() == null)
                .toList();
        }
